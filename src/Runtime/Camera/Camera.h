...
public:
  ...
  explicit Camera(const vec3 &pos = vec3(2.0f, 2.0f, -5.0f),
      ...
      : forward(vec3(0.0f, 0.0f, 1.0f)), up(vec3(0.0f, 1.0f, 0.0f)), fov(fov),
        _pos(pos), _projection(perspective(radians(fov), aspect, zNear, zFar)),
        _direction(vec3(-0.5f, -0.5f, 1.0f)), _zNear(zNear), _zFar(zFar) {}
