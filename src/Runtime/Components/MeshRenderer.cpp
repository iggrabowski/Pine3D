...
MeshRenderer::~MeshRenderer() { delete _model3D; }
...
int MeshRenderer::GetAttributeLocation(const std::string &attName) {
  if (!_shadersValidated) {
    Logger::Instance().Warning("MeshRenderer: Attempting to get attribute "
  ...
  Logger::Instance().Warning("MeshRenderer: Attribute '" + attName +
                             "' not found. [GetAttributeLocation()]");
  return -1;
}
...
bool MeshRenderer::ValidateShaderAttributes() {
  ...
  for (unsigned int i = 0; i < _model3D->num_materials; i++) {
    ...
    if (!mat) {
      Logger::Instance().Error("MeshRenderer: Material " + std::to_string(i) +
                               " - missing material.");
      return false;
    }
    ...
    for (const auto &attr : attrs) {
      // Check by location: if another attribute already uses this location it
      // must match name+type
      ...
      if (it_loc != loc_map.end()) {
        ...
        if (existing->name != attr.name || existing->type != attr.type) {
          Logger::Instance().Error(
              "MeshRenderer: Attribute conflict"
              " at location " +
              std::to_string(attr.loc) + ". Existing: '" + existing->name +
              "' (type " + std::to_string(existing->type) + "), Found: '" +
              attr.name + "' (type " + std::to_string(attr.type) + ").");
          return false;
        }
      } else {
      ...
      if (it_name != name_map.end()) {
        ...
        if (existing->loc != attr.loc || existing->type != attr.type) {
          Logger::Instance().Error(
              "MeshRenderer: Attribute conflict for name '" + attr.name +
              "'. Existing: location " + std::to_string(existing->loc) +
              " (type " + std::to_string(existing->type) +
              "), Found: location " + std::to_string(attr.loc) + " (type " +
