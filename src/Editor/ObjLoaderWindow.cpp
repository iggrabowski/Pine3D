...
ObjLoaderWindow::ObjLoaderWindow(const std::string &startDir) {
  ...
  Logger::Instance().Info("ObjLoaderWindow: Starting directory set to " +
                          CurrentDir.string());
  RefreshDirectory();
}
...
void ObjLoaderWindow::Open(const std::string &startDir) {
  ...
  Logger::Instance().Info("ObjLoaderWindow: Opening directory " +
                          CurrentDir.string());
  RefreshDirectory();
}

float ObjLoaderWindow::GetHeight() { return height; }
...
void ObjLoaderWindow::RefreshDirectory() {
  ...
  try {
    ...
    Entries.clear();
    Logger::Instance().Error("ObjLoaderWindow: RefreshDirectory error: " +
                             std::string(e.what()));
  }
}

void ObjLoaderWindow::Show(bool p_open) {
  // Position the ImGui window at the top-left of the main viewport and make it
  // fixed (not movable). Use WorkPos to respect platform menu/toolbar if any.
  ...
  // Add NoMove so user cannot drag it; keep AlwaysAutoResize and NoScrollbar as
  // before.
  ...
  if (ImGui::Button("Go")) {
    try {
      CurrentDir = std::filesystem::path(PathEditBuf);
      RefreshDirectory();
    } catch (...) {
    }
  }
  ...
  if (ImGui::Button("Up")) {
    ...
    std::snprintf(PathEditBuf, sizeof(PathEditBuf), "%s",
  ...
  ImGui::SameLine();
  // ImGui::TextWrapped("%s", SelectedPath.empty() ? "<none>" :
  // SelectedPath.c_str());
