...
void InputHandler::AddActionCallback(const std::string &actionName,
                                     const ActionCallback &callback) {
  _actionCallbacks[actionName].emplace_back(callback);
}

void InputHandler::RemoveActionCallback(const std::string &actionName,
                                        const std::string &callbackRef) {
  std::erase_if(_actionCallbacks[actionName],
                [callbackRef](const ActionCallback &callback) {
                  return callback.ref == callbackRef;
                });
}

void InputHandler::MapInputToAction(const KeyCode key,
                                    const InputAction &action) {
  _inputActionMapping[key].emplace_back(action);
}

void InputHandler::UnmapInputFromAction(const KeyCode key,
...
void InputHandler::ProcessInput() {
  ...
  for (auto &input_device : _inputDevices) {
    ...
    for (const auto &[first, second] : new_state) {
      for (const auto &action_event : GenerateActionEvent(
               input_device->index, first, input_device->states[first].value,
               second.value)) {
...
void InputHandler::UnregisterDevice(int deviceIndex, InputDeviceType type) {
  erase_if(_inputDevices,
           [type, deviceIndex](const std::shared_ptr<InputDevice> &device) {
             return device->type == type && device->index == deviceIndex;
           });
}

std::vector<InputHandler::ActionEvent>
InputHandler::GenerateActionEvent(const int deviceIndex, const KeyCode keyCode,
                                  const float oldVal, const float newVal) {
  std::vector<ActionEvent> action_events;
    ...
    if (constexpr float epsilon = 1e-5f; abs(newVal - oldVal) < epsilon) {
      ...
      if (abs(1.0f - newVal) < epsilon) {
        if (action.type & KEY_ON_HOLD) {
          action_events.emplace_back(
              ActionEvent{.device_index = deviceIndex,
...
bool InputHandler::IsMouseButtonPressed(MouseButtons mouseCode) {
  if (glfwGetMouseButton(Application::window->GetWindow(),
