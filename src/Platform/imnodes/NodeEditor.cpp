...
void NodeEditor::CreateMaterialComponentNode(TextureType type,
                                             Material &material,
                                             UiNode &ui_node) {
  ImVec2 position;
  ...
  component_node.id = graph_.insert_node(
      Node(NodeType::image, component_node.texture->GetGLHandle()));

  nodes_.push_back(component_node);
  ImNodes::SetNodeScreenSpacePos(component_node.id, position);
  switch (type) {
  case TEX_TYPE_BASE:
    graph_.insert_edge(ui_node.ui.material.input1, component_node.id);
    break;
  case TEX_TYPE_NORMAL:
    graph_.insert_edge(ui_node.ui.material.input2, component_node.id);
    break;
  case TEX_TYPE_METALLIC:
    graph_.insert_edge(ui_node.ui.material.input3, component_node.id);
    break;
  case TEX_TYPE_ROUGHNESS:
    graph_.insert_edge(ui_node.ui.material.input4, component_node.id);
    break;
  case TEX_TYPE_AO:
    graph_.insert_edge(ui_node.ui.material.input5, component_node.id);
    break;
  case TEX_TYPE_HEIGHT:
    graph_.insert_edge(ui_node.ui.material.input6, component_node.id);
    break;
  }
}
...
void NodeEditor::AddMaterialNodes(Material *material, ImVec2 pos) {
  if (!nodes_.empty()) {
    ...
    for (const UiNode &n : nodes_) {
      ...
      case UiNodeType::image:
        // image nodes have no internal child nodes in the graph (only the node
        // id)
        break;
  ...
  ImNodes::SetNodeScreenSpacePos(
      ui_node.id, ImVec2(BASE_NODE_POS_X + 430, BASE_NODE_POS_Y + 250));
...
void NodeEditor::Show() {
  ...
  if (root_node_id_ != -1) {
    ...
    if (root_ui_node->material != nullptr) {
      ...
      if (ImGui::CollapsingHeader("Material Properties (Mockup)",
        ...
        ImGui::SliderFloat("Metalness", &root_ui_node->material->m_metallic,
                           0.02f, 1.0f, "%.2f");
        ImGui::EndDisabled();
        ...
        ImGui::SliderFloat("Roughness", &root_ui_node->material->m_roughness,
                           0.02f, 1.0f, "%.2f");
        ImGui::EndDisabled();
      }
    }
  ...
  {
    const bool
        open_popup = // ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows)
                     // &&
        false; // ImNodes::IsEditorHovered() &&
    ...
    if (ImGui::BeginPopup("add node")) {
      ...
      if (ImGui::MenuItem("image")) {
        ...
        // create graph id with NodeType::image so linking logic treats output
        // port type different from plain value nodes
      ...
      // Content: input for path and thumbnail + load button
      {
        ...
        if (!node.imageTexturePath.empty())
          std::snprintf(pathBuf, sizeof(pathBuf), "%s",
                        node.imageTexturePath.c_str());

        ImGui::PushItemWidth(node_width);
        // if (ImGui::InputText(("Path##img" + std::to_string(node.id)).c_str(),
        // pathBuf, sizeof(pathBuf))) { /* intentionally commented: path editing
        // via UI disabled */ }
        ImGui::PopItemWidth();
    ...
    case UiNodeType::material: {
      ...
      ImGui::Dummy(ImVec2(node_width, 0.f));
      {
        ...
        if (graph_.num_edges_from_node(node.ui.material.input1) == 0ull) {
          ...
          ImGui::PushItemWidth(node_width - label_width);
          ImGui::DragFloat("##hidelabel",
                           &graph_.node(node.ui.material.input1).value, 0.01f,
                           0.f, 1.0f);
          ImGui::PopItemWidth();
        }
        ImNodes::EndInputAttribute();
...

      {
        ...
        if (graph_.num_edges_from_node(node.ui.material.input2) == 0ull) {
          ...
          ImGui::PushItemWidth(node_width - label_width);
          ImGui::DragFloat("##hidelabel",
                           &graph_.node(node.ui.material.input2).value, 0.01f,
                           0.f, 1.0f);
          ImGui::PopItemWidth();
        }
        ImNodes::EndInputAttribute();
...

      {
        ...
        if (graph_.num_edges_from_node(node.ui.material.input3) == 0ull) {
          ...
          ImGui::PushItemWidth(node_width - label_width);
          ImGui::DragFloat("##hidelabel",
                           &graph_.node(node.ui.material.input3).value, 0.01f,
                           0.f, 1.0f);
          ImGui::PopItemWidth();
        }
        ImNodes::EndInputAttribute();
...

      {
        ...
        if (graph_.num_edges_from_node(node.ui.material.input4) == 0ull) {
          ...
          ImGui::PushItemWidth(node_width - label_width);
          ImGui::DragFloat("##hidelabel",
                           &graph_.node(node.ui.material.input4).value, 0.01f,
                           0.f, 1.0f);
          ImGui::PopItemWidth();
        }
        ImNodes::EndInputAttribute();
  ...
  for (const auto &edge : graph_.edges()) {
    ...
    if (graph_.node(edge.from).type != NodeType::value &&
        graph_.node(edge.from).type != NodeType::image)
      continue;
  ...
  // const ImU32 color =
  //     root_node_id_ != -1 ? evaluate(graph_, root_node_id_) : IM_COL32(255,
  //     20, 147, 255);
