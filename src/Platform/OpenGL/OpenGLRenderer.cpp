...
void OpenGLRenderer::InitSkybox() {
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    glTextureSubImage3D(_skyboxTextureObj,
                        ...
                        0, // yOffset
                        i, // zOffset (layer)
                        static_cast<GLsizei>(faceWidth),
                        static_cast<GLsizei>(faceHeight), // 2D image dimensions
                        1,                                // depth
                        GL_RGB,                           // format
                        GL_FLOAT,                         // data type
                        pSrc);
...
void OpenGLRenderer::DrawSkybox() {
  ...
  if (skyboxShader) {
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, _skyboxTextureObj);
    // Use your shader helper to set an int sampler uniform; fallback to
    // glUniform1i if necessary:
    skyboxShader->SetUniformTextureSampler2D("u_skybox",
                                             0); // or SetUniform("u_skybox", 0)
  }

  // Draw a unit cube (36 verts). Create and keep a VAO/VBO for this cube in
  // InitSkybox. _skyboxCubeVAO is expected to be created earlier in InitSkybox.
  glBindVertexArray(_skyboxCubeVAO);
  glDrawArrays(GL_TRIANGLES, 0, 36);
  glBindVertexArray(0);
...
void OpenGLRenderer::InitPingPongBuffers() {
  glGenFramebuffers(2, _pingpongFBO);
  glGenTextures(2, _pingpongCBO);
